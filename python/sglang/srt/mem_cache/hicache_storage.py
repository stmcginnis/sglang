...
def get_hash_str(token_ids: List[int], prior_hash: Optional[str] = None) -> str:
    # a strawman implementation for demonstration purposes
    token_bytes = ",".join(map(str, token_ids)).encode("ascii")
    hasher = hashlib.sha256()
    if prior_hash:
        hasher.update(prior_hash.encode("ascii"))
...
class HiCacheFile(HiCacheStorage):
    ...
    def set(self, key: str, value: Tensor) -> bool:
        ...
        if self.exists(key):
            logger.warning(
                f"Key {key} already exists. Overwriting the existing tensor."
            )
        try:
